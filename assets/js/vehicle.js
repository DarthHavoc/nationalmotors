...
var hub=sold?['sold.html','Sold']:['inventory.html','Inventory'];
document.getElementById('crumb').innerHTML='<a href="index.html">Home</a> / <a href="'+hub[0]+'">'+hub[1]+'</a> / '+(v?v.year+' '+v.make+' '+v.model:'Vehicle');
document.querySelectorAll('.nav .n').forEach(function(a){var h=a.getAttribute('href')||'';if(h==='inventory.html'||h==='sold.html')a.classList.toggle('active',h===hub[0]);});
if(!v){el.innerHTML='<div class="empty">Vehicle not found. <a class="link" href="inventory.html">View inventory</a></div>';}
 ...
 var vinTxt=v.vin?(sold?vinmask(v.vin):v.vin):'\u2014';
 ...
 panel+='<div class="vp-ids"><span>Stock #'+v.id+'</span><span>VIN '+vinTxt+'</span></div><hr class="vp-div"><dl class="vp-specs">'+s6.map(function(x){return '<div><dt>'+x[0]+'</dt><dd>'+x[1]+'</dd></div>';}).join('')+'</dl>';
 ...
 else{panel+='<hr class="vp-div"><div class="vp-cta"><a class="btn fill block" href="inventory.html">View Inventory</a><a class="btn block" href="'+findurl(v)+'">Find Me One</a></div>';}
 panel+='</div>';
 el.innerHTML='<div class="vdp">'+gal+panel+'</div>';
 var fspec=[['Mileage',v.miles?miles(v.miles):'\u2014'],['Engine',v.engine||'\u2014'],['Transmission',v.trans||'\u2014'],['Drivetrain',v.drive||'\u2014'],['Fuel',v.fuel||'\u2014'],[(v.ev?'Range':'Economy'),v.range||'\u2014'],['Exterior',v.color||'\u2014'],['Interior',v.interior||'\u2014'],['Stock #',v.id],['VIN',vinTxt]];
 ...
 if(real&&N>1&&galMain&&!reduce){
  var slideTimer;
  function startSlide(){slideTimer=setInterval(function(){show(gi+1);},4500);}
  function resetSlide(){clearInterval(slideTimer);startSlide();}
  ['gprev','gnext','viewall'].forEach(function(id){var b=document.getElementById(id);if(b)b.addEventListener('click',resetSlide);});
  var th=document.querySelector('.thumbs');if(th)th.addEventListener('click',resetSlide);
  galMain.addEventListener('mouseenter',function(){clearInterval(slideTimer);});
  galMain.addEventListener('mouseleave',resetSlide);
  startSlide();
 }
